    ...
    def all_docs():
        ...
        if os.path.isfile(filename):
            with open(filename, 'rb') if filename != '-' else sys.stdin as doc_file:
                ...
                count = 0
                #source = filename.encode("utf-8").split('.')[0];
                for row in reader:
                    #row['source'] = source
                    count += 1
                    if count % 1000000 == 0:
            ...
            for i in os.listdir(filename):
                echo(i, quiet)
                if os.path.isfile(os.path.join(filename,i)) and i != '.DS_Store':
                    with open(os.path.join(filename,i), 'rb') if i != '-' else sys.stdin as doc_file:
                        # delimited file should include the field names as the first row
                        fieldnames = doc_file.next().strip().split(delimiter)
                        # echo('Using the following ' + str(len(fieldnames)) + ' fields:', quiet)
                        # for fieldname in fieldnames:
...
@retry(wait_exponential_multiplier=10, wait_exponential_max=100, stop_max_attempt_number=10)
...
@click.option('--delimiter', default=',', required=False,
